"""
CIA Triad Simulations (Confidentiality, Integrity, Availability)
Run: python cia_sim.py
"""
...
def integrity_demo(message: str) -> None:
    ...
    # Tamper with message (simple single-char tamper)
...
def availability_demo(
    ...
    for t in range(1, total_ticks + 1):
        ...
        # generate traffic (legit + attack); attack starts mid-sim
        incoming = legit_rps + (attack_rps if t > total_ticks // 2 else 0)
        ...
        for _ in range(retry_count_this_tick):
            req_id, attempts_left = pending_retries.popleft()
            if attempts_left <= 0:
                # no attempts left, drop it
                continue
            if rl.allow():
                dispatch(req_id)
                stats["sent"] += 1
                ...
                stats["dropped_rl"] += 1
                # put it back with same attempts_left (we didn't decrement because we didn't send)
                pending_retries.append((req_id, attempts_left))
        ...
        for _ in range(incoming):
            ...
            if rl.allow():
                ...
                stats["dropped_rl"] += 1
                # queue a retry if allowed
                if retry_attempts > 0:
                    pending_retries.append((req_counter, retry_attempts))

        # process each server's queue and collect server-level outcomes
        for s in pool:
            before_processed = s.processed
            before_failed = s.failed
            s.process_tick()
            processed_now = s.processed - before_processed  # successes this tick
            failed_now = s.failed - before_failed          # failures this tick

            # FIX: Add successes and failures independently (previously subtracted failures twice)
            stats["processed"] += processed_now
            stats["failed"] += failed_now

        # schedule retry tickets for newly failed requests (simplified)
        newly_failed = stats["failed"] - stats.get("_prev_failed", 0)
        stats["_prev_failed"] = stats["failed"]
        for _ in range(newly_failed):
            # synthesize a new req_id to represent a retry attempt
            req_counter += 1
            if retry_attempts > 0:
                # push with one fewer remaining attempt
